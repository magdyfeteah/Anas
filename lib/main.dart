...
//   bool isSpeaking = false; // Track if the bot is speaking
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    // ✅ استمع لحالة الصوت مرة واحدة فقط
    audioPlayer.onPlayerStateChanged.listen((state) {
      if (!mounted) return;
      if (state == PlayerState.playing) {
        setState(() => isSpeaking = true);
  ...
  Future<void> _sendAudioToServer(String filePath) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final tempFile = File('${(await getTemporaryDirectory()).path}/response_audio.wav');
        await tempFile.writeAsBytes(bytes);
        await audioPlayer.play(DeviceFileSource(tempFile.path));
        setState(() {
          text = '';
  ...
  Future<void> getAnswerAudio({required String answer}) async {
    ...
    final bytes = base64Decode(response.body.replaceAll('"', ''));
    final tempFile = File('${(await getTemporaryDirectory()).path}/response_audio.wav');
    await tempFile.writeAsBytes(bytes);
    await audioPlayer.play(DeviceFileSource(tempFile.path));
  }
  ...
  String adaptiveText() {
    if (isRecording) return "...أسمعك";
    if (isSpeaking) return "انس يتحدث...";
    return text;
  }
                        ...
                        GestureDetector(
                          onTap: () async {
                            if (isRecording) {
                              ...
                              if (await audioRecorder.hasPermission()) {
                                final filePath =
                                    "${(await getDownloadsDirectory())!.path}/recording.wav";
                                await audioRecorder.start(
                                  const RecordConfig(encoder: AudioEncoder.wav),
                                  path: filePath,
                                );
                          ...
                          child: Container(
                            padding: EdgeInsets.all(10),
                            decoration: BoxDecoration(
                              color: isRecording ? Colors.red : Color(0xFFD9D9D9),
                              shape: BoxShape.circle,
                            ),
                            child: SvgPicture.asset(
                              isRecording
                          ...
                          children: [
                            GestureDetector(
                              ...
                              child: Container(
                                padding: EdgeInsets.all(15),
                                decoration: onMic
                                    ? BoxDecoration(color: Color(0xFF1A1818), shape: BoxShape.circle)
                                    : null,
                                child: SvgPicture.asset(
                                  'assets/images/mic.svg',
                                  width: 30,
                                  height: 30,
                                  color: onMic ? Colors.white : Color(0xFF1A1818),
                                ),
                              ),
                            ),
                            GestureDetector(
